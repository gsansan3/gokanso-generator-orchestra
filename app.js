(() => {
    ...
    // 結果の効果音は、カードが開く瞬間に、ハープの4音（タラらん）で始まる。
    // その最後の音と同時に、和音（チャーン）が鳴る。ハープの最初の音から、和音までの時間が、これ。
    const RESULT_LEAD_MS = 150;
    ...
    const Sound = (() => {
        ...
        // 結果音の頭に鳴らす、ハープの4音（D4 F#4 A4 D5）と、鳴らし始める時刻（秒）。最後の音が和音と同時
        const HARP_NOTES = [293.66, 369.99, 440.0, 587.33];
        const HARP_DELAYS = [0, 0.04, 0.08, RESULT_LEAD_MS / 1000];
        ...
        return {
            ...
            result() {
                // ハープ: D-dur の4音（D4 F#4 A4 D5）を、カードが開く瞬間から、タラらんと駆け上がる。最後の音が、和音と重なる
